...
import { getFirestore, doc, setDoc, updateDoc, getDoc, arrayUnion, collection, getDocs, Timestamp } from "https://www.gstatic.com/firebasejs/9.23.0/firebase-firestore.js";
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  coachLogoutBtn.addEventListener("click", () => signOut(auth));

  // Canvas click for scoring
  ...
  // Load user role and show correct menu
  async function loadUserRole(){
    ...
    if(docSnap.exists()){
      ...
      if(role === "archer"){
        ...
        showScreen("coachPanel");
        loadCoachData();
      }
    }
  }

  // Start session (archer)
  ...
  function showBadges(){
    ...
    // Example badges
    if(currentSession.totalScore === currentSession.endsCount * currentSession.arrowsPerEnd * 10){
      badges.push("🏆 Perfect Session!");
    }
    if(currentSession.ends.some(end=>end.every(a=>a===10))){
      badges.push("🎯 End of X's!");
    }
  ...
  // Coach: Load all archers
  async function loadCoachData(){
    const usersSnap = await getDocs(collection(db,"users"));
    const archersDiv = document.getElementById("archersList");
    archersDiv.innerHTML = "";
    usersSnap.forEach(u=>{
      const data = u.data();
      if(data.role === "archer"){
        const card = document.createElement("div");
        card.className = "archerCard";
        card.innerHTML = `<h3>${data.name}</h3><p>Total Sessions: ${data.sessions.length}</p>`;
        archersDiv.appendChild(card);
      }
    });
  }
